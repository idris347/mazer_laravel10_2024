...
<div class="page-title">
    <div class="row">
        <div class="col-12 col-md-6 order-md-1 order-last">
            <h3>Crud Resource</h3>
            <p class="text-subtitle text-muted">terlihat sama tampilannya namun akan berbeda dari sisi kode routenya dan lebih efisien (bisa di lihat dari urlnya)</p>
        </div>
...
<div class="card">
    ...
    <div class="card-header">
        <a href="{{ route('crudapis.create') }}" class="btn btn-primary rounded-pill">Tambah data</a>
    </div>
    @php
        $no = 1;
    @endphp
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{route('crudapis.edit',$item->id)}}" class="btn icon btn-primary"><i class="bi bi-pencil"></i></a>
                                <button type="button" class="btn icon btn-danger delete-product" data-product-id="{{ $item->id }}"><i class="bi bi-trash"></i></button>
                                <form id="delete-form-{{$item->id}}" action="{{ route('crudapis.destroy',$item->id) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('DELETE')
